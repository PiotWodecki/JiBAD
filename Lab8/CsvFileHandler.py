...
class CsvFileStorage:
    ...
    def __init__(self, description_row=None, label=None):
        self.description_row = description_row  # skąd ta nazwa?
        self.label = label

    def read_csv_file(self, path, delimiter=';', label_col=-1):
        with open(path, 'r') as file:   # nie powinno się nadpisywać nazw wbudowanych
            data = csv.reader(file, delimiter=delimiter)
            data = [d for d in data]    # <=> list(data)

        data = np.asarray(data, float)
        if isinstance(label_col, (int, float)): # indeks kolumny jako float?
    ...
    def normalize_column(self, col):
        self.description_row[:, col] = (self.description_row[:, col] - self.description_row[:, col].min())/\
                                       (self.description_row[:, col].max() - self.description_row[:, col].min())    # dwa razy liczone minimum

    def normalize_all_rows(self):
        for i in range(self.description_row.shape[0]):  # numpy wykonuje operacje element po elemencie nieporównywalnie szybciej niż działa taka pętla
